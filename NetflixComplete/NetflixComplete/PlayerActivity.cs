...
namespace NetflixComplete.DroidTV
{
	...
	{ 
		...
		private void StartVideoPlayer ()
		{
			...
			mSelectedMovie = Newtonsoft.Json.JsonConvert.DeserializeObject<Video>(this.Intent.GetStringExtra("SelectedMovie"));
			if (null != b) {
		...
		public bool OnError(MediaPlayer mp, MediaError error, int what){
			...
			if (error == MediaError.TimedOut) {
				msg = "The video could not be loaded quickly enough.";
			} else if (error == MediaError.ServerDied) {
				msg = "Netflix could not be reached. Please check your Internet connection.";
			} else {
				msg = "The video could not be loaded.";
			}
		...
		internal class BackToDetailTask : TimerTask
		{
			...
			{
				owner.mHandler.Post (() => {
					Intent intent = new Intent (owner, Utils.ToJavaClass (typeof(VideoDetailsActivity)));
					intent.PutExtra (owner.Resources.GetString (Resource.String.movie), Newtonsoft.Json.JsonConvert.SerializeObject(owner.mSelectedMovie));
